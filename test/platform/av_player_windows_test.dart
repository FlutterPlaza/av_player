  ...
  group('AvPlayerWindows', () {
    ...
    test('uses correct method channel name', () {
      expect(
        ...
        'com.flutterplaza.av_player_windows',
      );
    });
    ...
    test('isPipAvailable() always returns false', () async {
      final available = await platform.isPipAvailable();
      expect(available, false);
      expect(log, isEmpty);
    });

    test('enterPip() is a no-op', () async {
      await platform.enterPip(1);
      expect(log, isEmpty);
    });

    test('enterPip() with aspect ratio is a no-op', () async {
      await platform.enterPip(1, aspectRatio: 16 / 9);
      expect(log, isEmpty);
    });

    test('exitPip() is a no-op', () async {
